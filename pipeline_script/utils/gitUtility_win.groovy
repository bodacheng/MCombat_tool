...
def getGitRevision() {
    ...
    def gitRevisionFull = bat(script:'git rev-parse HEAD', returnStdout:true)
...
def getGitLogMessage(branch) {
    return bat(script: "git log origin/${branch} --pretty=short -1", returnStdout: true)
}

// コミットメッセージ情報の取得
def getGitCommitMessage() {
    def commitMessage = bat(script:'git log --format=format:%s -1', returnStdout:true)
    return commitMessage
}
// コミットメッセージ情報の取得
def getGitCommitAuthor() {
    def author = bat(script:'git log --format=format:%an -1', returnStdout:true)
    return author
}

//コミット日時の取得
def getGitCommitDate() {
    def date = bat(script:'git log --date=format-local:"%Y/%m/%d %H:%M:%S" --format=format:"%cd" -1', returnStdout:true)
    return date
}
